...
function populateZone(polygon, opts) {
  ...
  var targetSites = opts.siteCount != null
    ? opts.siteCount
    : Math.max(4, Math.min(40, Math.round(area / (minSpacing * minSpacing * 3.2))));
  ...
  var buildSites = [];
  var nBuild = Math.max(sites.length ? 1 : 0, Math.round(sites.length / 3));
  ...
  // ----- props along roads -----
  var props = [];
  var propStride = opts.propStride != null ? opts.propStride : minSpacing * 0.9;
...
function buildZone(description, opts) {
  ...
  group.add(makePlaza(description.plaza, plazaR, M));
