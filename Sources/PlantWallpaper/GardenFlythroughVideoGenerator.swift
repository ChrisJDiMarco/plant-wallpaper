...
struct GardenFlythroughVideoGenerator: Sendable {
    ...
    static func referenceRequestBodyForSelfTest(startFrameURL: URL, originalMapURL: URL, pathInstruction: String?, segmentCount: Int = 6) -> [String: Any] {
    ...
    private static func imagePrompt(
        ...
        if usesPolicyFallback {
            return appendPathInstruction(
                to: policyFallbackPrompt(segmentIndex: segmentIndex, segmentCount: segmentCount, isFinalSegment: true),
                pathInstruction: pathInstruction
            )
        }
        if segmentCount <= 1 {
            return appendPathInstruction(to: loopPrompt, pathInstruction: pathInstruction)
        }

        let totalDurationSeconds = segmentCount * 10
        let prompt = """
        ...
        Use the provided image as the exact first frame for this 10-second segment.
    ...
    private static func referencePrompt(
        ...
        if usesPolicyFallback {
            return appendPathInstruction(
                to: policyFallbackPrompt(segmentIndex: segmentIndex, segmentCount: segmentCount, isFinalSegment: false),
                pathInstruction: pathInstruction
            )
        }
        let totalDurationSeconds = segmentCount * 10
        ...
        Use @Image1 as the exact first frame for this 10-second segment. Use @Image2 as the persistent full-garden map and continuity reference for the entire scene.
    ...
    private static func policyFallbackPrompt(segmentIndex: Int, segmentCount: Int, isFinalSegment: Bool) -> String {
        let totalDurationSeconds = segmentCount * 10
        let ending = isFinalSegment
            ...
            : "End on a stable garden frame that can continue into the next 10-second segment."
        return """
