...
function addHouseDetail(group, type, opts) {
  ...
  var matLeaf      = _detailMat(0x4d7e45, 'leaf',    { roughness: 0.86 });
  var matPetal     = _detailMat(0xd96b83, 'petal',   { roughness: 0.82 });
  var matPetalGold = _detailMat(0xf2ce61, 'petal',   { roughness: 0.82 });
  var matLinenBlue = _detailMat(0x8fb5d9, 'linen',   { roughness: 0.94 });
  var matLinenTan  = _detailMat(0xe6d3a6, 'linen',   { roughness: 0.96 });
  var matHearth    = _detailMat(0xffdd8a, 'glass',   { emissive: 0xffbd55, emissiveIntensity: 0.6, transparent: true, opacity: 0.76, roughness: 0.35 });
  ...
  winSpecs.forEach(function (s) {
    ...
    planter.position.set(0, -2.8, 0.5);
    _dAdd(planter, new THREE.BoxGeometry(4.7, 0.7, 1.0), matWoodDark, 0, 0, 0);
    for (var lp = 0; lp < 5; lp++) {
      var leaf = _dAdd(planter, new THREE.SphereGeometry(0.28, 8, 6), matLeaf, -1.8 + lp * 0.9, 0.46 + (lp % 2) * 0.14, 0.25);
      leaf.scale.set(1.2, 0.62, 0.8);
      if (lp % 2 === 0) _dAdd(planter, new THREE.SphereGeometry(0.18, 8, 6), lp % 4 === 0 ? matPetal : matPetalGold, leaf.position.x, leaf.position.y + 0.32, 0.42);
    }
    win.add(planter);
    group.add(win);
  });

  var hearthGlow = _dAdd(group, new THREE.SphereGeometry(1.25, 16, 10), matHearth, 0, winY, frontZ + 0.52);
  hearthGlow.name = 'hearthGlow';
  hearthGlow.scale.set(1.8, 1.2, 0.35);
  ...
  for (var st = 0; st < 5; st++) {
    var stone = _dAdd(group, new THREE.CylinderGeometry(1.15, 1.3, 0.32, 9), matStone, (st - 2) * 1.2, 0.18, frontZ + 3.2 + st * 1.2);
    stone.rotation.y = st * 0.37;
    stone.scale.x = 1.2 - (st % 2) * 0.18;
    stone.scale.z = 0.72 + (st % 3) * 0.09;
  }

  /* ---- tiny domestic life: kindling, low fence, laundry strip ---- */
  var kindling = new THREE.Group();
  kindling.position.set(-dW * 0.9, 0, frontZ + 1.2);
  for (var kl = 0; kl < 5; kl++) {
    var stick = _dAdd(kindling, new THREE.CylinderGeometry(0.16, 0.2, 3.6, 6), matWoodDark, 0, 0.9 + kl * 0.18, 0);
    stick.rotation.set(Math.PI / 2, 0, -0.42 + kl * 0.18);
  }
  group.add(kindling);

  [-1, 1].forEach(function (side) {
    for (var fp = 0; fp < 3; fp++) {
      _dAdd(group, new THREE.CylinderGeometry(0.16, 0.22, 2.6, 7), matWoodDark, side * (dW * 1.05 + fp * 1.2), 1.3, frontZ + 2.7 + fp * 0.08);
    }
    var rail = _dAdd(group, new THREE.BoxGeometry(3.7, 0.22, 0.22), matWoodLight, side * (dW * 1.38), 1.9, frontZ + 2.75);
    rail.rotation.z = side * 0.04;
  });

  var laundry = new THREE.Group();
  laundry.position.set(dW * 1.15, 0, -frontZ * 0.55);
  [-1, 1].forEach(function (side) {
    _dAdd(laundry, new THREE.CylinderGeometry(0.14, 0.18, 4.0, 7), matWoodDark, side * 3.2, 2.0, 0);
  });
  var laundryLine = _dAdd(laundry, new THREE.CylinderGeometry(0.045, 0.045, 6.4, 5), matWoodLight, 0, 3.4, 0);
  laundryLine.rotation.z = Math.PI / 2;
  _dAdd(laundry, new THREE.BoxGeometry(1.1, 1.4, 0.08), matLinenBlue, -1.4, 2.6, 0.08);
  _dAdd(laundry, new THREE.BoxGeometry(1.0, 1.2, 0.08), matLinenTan, 0.7, 2.7, 0.08);
  group.add(laundry);
