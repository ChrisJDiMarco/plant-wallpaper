...
function makeGnome(opts) {
  ...
  var matSkin     = _M(SKIN,        { kind: 'skin', roughness: 0.6 });
  var matSkinD    = _M(SKIN_D,      { kind: 'skin', roughness: 0.6 });
  var matNose     = _M(NOSE,        { kind: 'skin', roughness: 0.55 });
  var matBeard    = _M(BEARD,       { kind: 'beard' });
  var matBeardD   = _M(BEARD_D,     { kind: 'beard' });
  var matBrow     = _M(BROW,        { kind: 'beard' });
  var matShirt    = _M(shirtColor,  { kind: 'linen' });
  var matTunic    = _M(tunicColor,  { kind: 'fabric' });
  var matTunicD   = _M(_darken(tunicColor, 0.8), { kind: 'fabric' });
  var matHat      = _M(hatColor,    { kind: 'felt' });
  var matHatD     = _M(_darken(hatColor, 0.82), { kind: 'felt' });
  var matLeather  = _M(LEATHER,     { kind: 'leather' });
  var matLeatherD = _M(LEATHER_D,   { kind: 'leather' });
  var matBuckle   = _M(BUCKLE,      { kind: 'metal', roughness: 0.35, metalness: 0.7 });
  var matTrouser  = _M(TROUSER,     { kind: 'wool' });
